// Web Audio API Synthesizer (100% Pure synthesized audio, no external files)
class SoundEngine {
  constructor() {
    this.ctx = null;
    this.muted = false;
  ...
  // Heartbeat sound for Situational Reflex
  heartbeat() {
    if (this.muted) return;
    this.init();
    if (!this.ctx) return;
  ...
  victory() {
    ...
    notes.forEach((f, idx) => {
      setTimeout(() => {
        this.playTone(f, 0.2, 'triangle', 0.2);
      }, idx * 100);
    });
  }
}
